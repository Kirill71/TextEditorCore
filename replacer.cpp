...
void Replacer::replaceInContainer(const position & current_pos, MyContainer & container, const std::string new_str){
	container[current_pos.m_row]
		.replace(current_pos.m_col, m_findText.searchString().length(), new_str);
	m_isSuccessfully = true;
}

bool Replacer::replace(const std::string & old_str, const std::string & new_str, const position& max_pos, MyContainer& container){
...
bool Replacer::replaceAll(const std::string & old_str, const std::string & new_str, const position& max_pos, MyContainer& container){
